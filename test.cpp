...
void TestRintP_(const int num) {
	...
	for (int iteration = 0; iteration < num; ++iteration) {
		...
		const std::vector<std::string>structures = EnumerateStructures(sequence, max_span, max_loop);
		std::uniform_int_distribution<int> length(0, int(structures.size()) - 1);
		const std::string str = structures[length(rnd)];
		const auto base_pairing_matrix = VerifyAndParseStructure(str, sequence, max_span, max_loop);
		const int max_dim = ComputeMaxHammingDistance(sequence, base_pairing_matrix, max_span, max_loop);
		//const auto ans1 = BruteForceRintP1Dim(sequence, str, max_dim, 37.0, max_span, max_loop);
		const auto ans2 = ComputeRintP1Dim<WideComplexNumber<IntervalVar>>(sequence, base_pairing_matrix, max_dim, 37.0, max_span, max_loop, false);
		const auto ans3 = RegularizeRintP1Dim(ans2.first, ans2.second);
		const auto ans4 = ComputeRintP1Dim<WideComplexNumber<Floating>>(sequence, base_pairing_matrix, max_dim, 37.0, max_span, max_loop, false);
		const auto ans5 = RegularizeRintP1Dim(ans4.first, ans4.second);

		double maxnum = 0.0;
		double maxdiff = 0.0;
		for (int d = 0; d <= max_dim; ++d) {
			if (!(ans3.first[d].lower() <= ans5.first[d] && ans5.first[d] <= ans3.first[d].upper())) {
				assert(ans3.first[d].lower() <= ans5.first[d] && ans5.first[d] <= ans3.first[d].upper());
			}
		}
		for (int d = 0; d <= max_dim; ++d) {
			for (int i = 1; i <= n; i++)for (int j = 0; j < 6; j++) {
				if (!(ans3.second[d][i][j].lower() <= ans5.second[d][i][j] && ans5.second[d][i][j] <= ans3.second[d][i][j].upper())) {
					assert(ans3.second[d][i][j].lower() <= ans5.second[d][i][j] && ans5.second[d][i][j] <= ans3.second[d][i][j].upper());
				}
			}
		}
	}
...
void TestRintP(const int num) {
	...
	for (int iteration = 0; iteration < num; ++iteration) {
		...
		const int max_dim = ComputeMaxHammingDistance(sequence, base_pairing_matrix, max_span, max_loop);
		const auto ans1 = BruteForceRintP1Dim(sequence, str, max_dim, 37.0, max_span, max_loop);
		const auto ans2 = ComputeRintP1Dim<WideComplexNumber<Floating>>(sequence, base_pairing_matrix, max_dim, 37.0, max_span, max_loop, false);
		const auto ans3 = RegularizeRintP1Dim(ans2.first, ans2.second);
		...
		for (int d = 0; d <= max_dim; ++d) {
			maxnum = std::max<double>(maxnum, abs(ans1.first[d]));
			maxdiff = std::max<double>(maxdiff, abs(ans1.first[d] - ans3.first[d]));
		}
		for (int d = 0; d <= max_dim; ++d) {
			for (int i = 1; i <= n; i++)for (int j = 0; j < 6; j++) {
				maxnum = std::max<double>(maxnum, abs(ans1.second[d][i][j]));
				maxdiff = std::max<double>(maxdiff, abs(ans1.second[d][i][j] - ans3.second[d][i][j]) * abs(ans1.first[d]));
			}
		}
		if (maxdiff >= maxnum * 0.0000000001) {
